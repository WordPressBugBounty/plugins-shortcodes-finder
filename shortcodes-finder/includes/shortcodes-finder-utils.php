 ...
 * @since    1.2.9
 */
function shortcodes_finder_get_posts_ids($post_type = 'post', $include_not_published = false)
{
 ...
 * @since    1.6.2
 */
function shortcodes_finder_get_shortcodes_in_content($type_name, $type_title = '', &$shortcodes = array(), $shortcode_to_search = '', $include_not_published = false)
{
	if ($type_title == '') {
	...
	$posts = get_posts($args);
	shortcodes_finder_get_shortcodes_by_shortcode($posts, $type_title, $shortcodes, $shortcode_to_search);
}
 ...
 * @since    1.6.2
 */
function shortcodes_finder_get_shortcodes_by_post_recursive($content, $post, $pattern, &$shortcodes)
{
	if (preg_match_all($pattern, $content, $matches) && array_key_exists(2, $matches)) {
		foreach ($matches[0] as $key => $value) {
			...
			// Recursivity
			shortcodes_finder_get_shortcodes_by_post_recursive($matches[5][$key], $post, $pattern, $shortcodes);
		}
	}
}
 ...
 * @since    1.6.2
 */
function shortcodes_finder_get_shortcodes_by_post($posts)
{
	$shortcodes = array();

	shortcodes_finder_import_third_party_shortcodes();

	$pattern = '/' . get_shortcode_regex() . '/s';

	foreach ($posts as $post) {
		shortcodes_finder_get_shortcodes_by_post_recursive($post->post_content, $post, $pattern, $shortcodes);
	}
	return $shortcodes;
}
 ...
 * @since    1.2.9
 */
function shortcodes_finder_get_shortcodes_by_shortcode_recursive($content, $post, $type_title, $pattern, &$shortcodes, $shortcode_to_search = '')
{
	if (preg_match_all($pattern, $content, $matches) && array_key_exists(2, $matches)) {
		foreach ($matches[0] as $key => $value) {
			...
			// Recursivity
			shortcodes_finder_get_shortcodes_by_shortcode_recursive($matches[5][$key], $post, $type_title, $pattern, $shortcodes, $shortcode_to_search);
		}
	}
}
 ...
 * @since    1.6.2
 */
function shortcodes_finder_get_shortcodes_by_shortcode($posts, $type_title, &$shortcodes = array(), $shortcode_to_search = '')
{
	shortcodes_finder_import_third_party_shortcodes();
	...
	foreach ($posts as $post) {
		shortcodes_finder_get_shortcodes_by_shortcode_recursive($post->post_content, $post, $type_title, $pattern, $shortcodes, $shortcode_to_search);
	}
}
 ...
 * @since    1.6.2
 */
function shortcodes_finder_get_shortcodes_unused_by_post($posts)
{
	 ...
	 * @since 1.4.0
	 **/
	function shortcodes_finder_remove_html_comments($content = '')
	{
		return preg_replace('/<!--(.|\s)*?-->/', '', $content);
	}

	$pattern = '/' . shortcodes_finder_get_shortcode_unused_regex() . '/s';

	foreach ($posts as $post) {
		if (preg_match_all($pattern, shortcodes_finder_remove_html_comments($post->post_content), $matches) && array_key_exists(2, $matches)) {
			foreach ($matches[0] as $key => $value) {
				$shortcode = array();
 ...
 * @since    1.6.2
 */
function shortcodes_finder_import_third_party_shortcodes()
{
	// WPBakery Visual Composer
 ...
 * @since    1.6.2
 */
function shortcodes_finder_get_shortcodes_ordered($load_origin = false, $load_attributes = false, $object_to_load = '')
{
	shortcodes_finder_import_third_party_shortcodes();
	...
	if (is_array($shortcode_tags)) {
		...
		foreach ($shortcode_tags as $tag => $handler_function) {
			...
			if ($load_origin) {
				$shortcode_origin = shortcodes_finder_admin_page_shortcode_origin($handler_function);
				...
				if ($object_name != 'error' && ($object_to_load == '' || $object_to_load == $shortcode_origin['tag'])) {
					...
					if ($load_attributes)
						$shortcode_origin['attributes'] = shortcodes_finder_shortcode_attributes_processor($tag);
 ...
 * @since    1.6.2
 */
function shortcodes_finder_admin_page_shortcode_origin($callback)
{
	...
	$shortcode_origin =	array(
		...
		'object'        => '', 			// set in shortcodes_finder_admin_page_get_all_shortcodes
		'type'          => $shortcode_type,
		'name'          => $shortcode_name,
		'attributes'    => array(),  	// set in shortcodes_finder_admin_page_get_all_shortcodes
 ...
 * @since    1.6.2
 */
function shortcodes_finder_shortcode_attributes_processor($shortcode)
{
	global $shortcodes_finder_admin_page_shortcode_atts;

	// Register filter for shortcode attributes
	$filter = 'shortcode_atts_' . $shortcode;

	add_filter($filter, 'shortcodes_finder_shortcode_attributes_collector', PHP_INT_MAX, 4);
	...
	// Unregister filter
	remove_filter($filter, 'shortcodes_finder_shortcode_attributes_collector', PHP_INT_MAX);

	if (isset($shortcodes_finder_admin_page_shortcode_atts[$shortcode]) && is_array($shortcodes_finder_admin_page_shortcode_atts[$shortcode])) {
		$atts = $shortcodes_finder_admin_page_shortcode_atts[$shortcode];
	} else {
		$atts = '';
	}
 ...
 * @since    1.6.2
 */
function shortcodes_finder_shortcode_attributes_collector($out, $pairs, $atts, $shortcode)
{
	global $shortcodes_finder_admin_page_shortcode_atts;

	$shortcodes_finder_admin_page_shortcode_atts[$shortcode] = $pairs;
}
 ...
 * @since    1.6.2
 */
function shortcodes_finder_get_objects_with_shortcodes($shortcodes)
{
 ...
 * @since    1.6.2
 */
function shortcodes_finder_get_shortcode_unused_regex()
{
	shortcodes_finder_import_third_party_shortcodes();
	global $shortcode_tags;
 ...
 * @since    1.3.2
 */
function shortcodes_finder_clear_content_from_shortcode_unused($content)
{
 ...
 * @since    1.6.2
 */
function shortcodes_finder_html_to_text($html)
{
